...
function ChangeLocationModal({ onClose }) {
    ...
    function showPosition(position) {
      ...
      async function getLocation() {
          ...
          const API_key = '1193fefae6b4a3a88882fc7cab6994cc';
          console.log(location.lat); 
          const info = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${location.lat}&lon=${location.lon}&appid=${API_key}` )
          const data= await info.json();  
          console.log(data.name);
          setplace((data.name));
          localStorage.setItem('location',!data.name?"NULL":data.name);
        } else { 
            setlocation({lat:"NIL",lot:"NIL"});
        }
    }
                        ...
                        <div className="max-w-md mx-5 w-[55%] xl:max-w-3xl lg:max-w-xl md:max-w-xl bg-white max-h-[93vh] shadow-lg rounded-lg relative">
                            ...
                            <div className="px-4 pb-3 mt-2">
                                <div className="text-xs text-[#65676B]">
                                            <div>
                                                <span className=" text-lg cursor-pointer" onClick={getLocation}>Locate Me</span>
                                                <button  className=" ">
                                                <GiClick className=" size-5"/>
                                                </button>
                                            </div>
                                    <div className="border-2 rounded-md mb-4">
                                     {/* to add location facility */}
                                        <div className="flex items-center gap-2">
                                            ...
                                            <FormInput
                                                ...
                                                placeholderText = {location.lat===""?"Enter a town or city":place}
                                            />
                                        </div>
                                        {/* // */}
                                    </div>
